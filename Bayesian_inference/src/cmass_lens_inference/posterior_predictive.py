...
DEFAULT_TREND_POSTERIOR_DRAWS = 256
...
def run_posterior_trends(
    ...
    sigma_table_path: str,
    output_root_dir: str,
    n_posterior_draws: int = DEFAULT_TREND_POSTERIOR_DRAWS,
    burn_in: str | int = "auto",
    ...
    The selection kernel itself remains the current local one. This function is
    only changing how the parent population is generated and aggregated.
    """
    ...
    posterior_draws, _ = _load_posterior_draws(
    ...
    trend_draws = {
        quantity_name: {
            category_name: np.full((n_posterior_draws, n_mass_bins), np.nan, dtype=float)
            for category_name in TREND_CATEGORY_NAMES
        }
        for quantity_name in TREND_QUANTITY_NAMES
    }
    parent_bin_counts_draws = np.zeros((n_posterior_draws, n_mass_bins), dtype=int)
    detectable_weight_sums_draws = np.zeros((n_posterior_draws, n_mass_bins), dtype=float)
    selected_weight_sums_draws = np.zeros((n_posterior_draws, n_mass_bins), dtype=float)
    ...
    for draw_index, theta in enumerate(posterior_draws):
        ...
        for quantity_name in TREND_QUANTITY_NAMES:
            ...
            for category_name in TREND_CATEGORY_NAMES:
                trend_draws[quantity_name][category_name][draw_index] = reduced[category_name]

            # The counts and weight sums do not depend on which quantity is being
            # reduced, so we store them once from the first quantity pass.
            if quantity_name == TREND_QUANTITY_NAMES[0]:
                parent_bin_counts_draws[draw_index] = reduced["parent_bin_counts"]
                detectable_weight_sums_draws[draw_index] = reduced["detectable_weight_sums"]
                selected_weight_sums_draws[draw_index] = reduced["selected_weight_sums"]
