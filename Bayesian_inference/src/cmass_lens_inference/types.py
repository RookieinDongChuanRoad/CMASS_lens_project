...
class PosteriorPredictiveResult:
    """
    Structured summary of one posterior predictive test execution.

    The PPC workflow writes multiple artifacts to disk and also returns a
    small machine-readable payload to the CLI caller. Keeping that contract in
    a dedicated dataclass makes the boundary explicit and avoids leaking
    implementation details such as internal numpy arrays into the public API.
    """
...
class PosteriorPredictiveMonitorResult:
    ...
    This result keeps the monitoring phase explicit instead of overloading the
    single-profile `PosteriorPredictiveResult`. The monitor coordinates two
    table files and two PPC executions, so callers need a first-class summary
    that records both readiness evidence and both downstream run results.
    """
...
class PosteriorTrendResult:
    """
    Structured summary of one Fig. 8-like posterior trend evaluation.

    This workflow is intentionally separate from the histogram-based posterior
    predictive checks. The returned contract needs to record different output
    artifacts, different sample counts, and the stellar-mass binning settings
    used to summarize the sampled parent-population trends.
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize the trend result into JSON-friendly data."""
...
class NotebookComparisonResult:
    ...
    This result is intentionally distinct from the production PPC dataclasses.
    The comparison run answers a historical/scientific debugging question and
    therefore needs to preserve its own contract without leaking those choices
    into the default PPC API.
    """
