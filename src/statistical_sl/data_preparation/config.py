...
def _discover_repository_root(start_path: Path) -> Path:
    """Find the checkout root that owns the package and workspace directories.

    Data-preparation code may run from an editable checkout during development
    or from an installed package during tests.  The canonical project workflow
    uses ``workspace/data`` rather than a top-level ``data`` directory, so the
    default path discovery must anchor on a repository/workspace boundary
    instead of walking a fixed number of parents from this module file.
    """

    for candidate in (start_path, *start_path.parents):
        if (candidate / "pyproject.toml").is_file() and (candidate / "workspace").is_dir():
            return candidate
    return Path.cwd()


PROJECT_ROOT = _discover_repository_root(Path(__file__).resolve())
WORKSPACE_ROOT = PROJECT_ROOT / "workspace"
WORKSPACE_DATA_DIRECTORY = WORKSPACE_ROOT / "data"
RAW_DATA_DIRECTORY = WORKSPACE_DATA_DIRECTORY / "raw"
EXTERNAL_DATA_DIRECTORY = WORKSPACE_DATA_DIRECTORY / "external"
CANONICAL_DATA_DIRECTORY = WORKSPACE_DATA_DIRECTORY / "canonical"
...
SIGMA_BUNDLE_GROUP_NAMES = SUPPORTED_OBSERVATION_FLAVORS + (WITHIN_RE_SIGMA_DEFINITION,)
