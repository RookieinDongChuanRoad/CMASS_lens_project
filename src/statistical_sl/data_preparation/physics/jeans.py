...
from statistical_sl.data_preparation.models import AperturePolicy, GalaxyInputs
from statistical_sl.core.unit_conventions import Sunit_hinv_from_fixed_kpc
...
def _load_spherical_jeans():
    """Import the optional external Jeans solver at computation time.

    The command-line parser and lightweight configuration tests should not fail
    just because the heavy scientific solver is missing from an environment
    that is only inspecting help text.  Real Jeans calculations still fail with
    a direct dependency error when the solver is required.
    """

    from spherical_jeans import sigma_model, tracer_profiles
    from spherical_jeans.mass_profiles import powerlaw

    return sigma_model, tracer_profiles, powerlaw
...
def _resolve_tracer_setup(
    ...
    _, tracer_profiles, _ = _load_spherical_jeans()
...
def _compute_sigma_unit_values_for_prepared_inputs(
    ...
    sigma_model, _, powerlaw = _load_spherical_jeans()
