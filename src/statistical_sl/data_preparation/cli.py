...
from statistical_sl.data_preparation.dataset_schema.writer import write_canonical_inference_dataset
from statistical_sl.data_preparation.io.boss_observations import build_boss_observation_hdf5_files
from statistical_sl.data_preparation.io.hdf5 import process_hdf5_file
from statistical_sl.data_preparation.io.lensing_cross_sections import (
    write_fibre_cross_section_hdf5,
    write_power_law_cross_section_hdf5,
)
from statistical_sl.data_preparation.io.slacs_observations import (
    write_slacs_observation_hdf5,
    write_slacs_population_sigma_unit_hdf5,
)
from statistical_sl.data_preparation.io.slit_observation_updates import sync_slit_canonical_updates
from statistical_sl.data_preparation.io.sigma_tables import (
    build_default_sigma_unit_hdf5_tables,
    repack_legacy_sigma_unit_hdf5_tables_into_bundles,
)
from statistical_sl.data_preparation.physics.lensing_cross_section import (
...
def build_parser() -> argparse.ArgumentParser:
    """Create the parser used by the `statistical-sl prepare-dataset` workflow."""
    ...
    parser.add_argument(
        "--all-default-inputs",
        action="store_true",
        help="Process the two standard project HDF5 files under workspace/data/raw/.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Override the BOSS summary-table path. Defaults to workspace/data/raw/{BOSS_SUMMARY_FILENAME}.",
    )
    parser.add_argument(
        "--workers",
        type=int,
