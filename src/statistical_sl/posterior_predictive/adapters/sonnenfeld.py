...
from statistical_sl.inference.canonical_context import lens_gamma_axis
...
from statistical_sl.numerics.numba.kernels.selection import theta_e_est_from_sigma_proxy
...
from statistical_sl.inference.types import ObservationRecord, RuntimeConfig
from statistical_sl.core.mass_definition import MassDefinition
from statistical_sl.posterior_predictive.backends.numba.diagnostics import (
...
def _run_sonnenfeld_parent_diagnostics(
    ...
    *,
    ...
    # The generic PPT runner records the execution policy, while the package
    # PPC backend owns applying that policy to process-local Numba execution.
    apply_diagnostics_thread_policy(execution, thread_limiter=apply_thread_limits)
    ...
    for chunk_start, chunk_end in iter_draw_chunks(n_draws=n_draws, chunk_size=chunk_size):
        ...
        chunk_outputs.append(
            _sonnenfeld_parent_diagnostics_numba_chunk(
                as_float64_contiguous(posterior_draws_array[chunk_start:chunk_end]),
                as_int64_contiguous(parent_indices),
                as_float64_contiguous(theta_uniforms),
                as_float64_contiguous(sigma_uniforms),
                as_float64_contiguous(context_arrays["scalar_context"]),
                as_float64_contiguous(context_arrays["parent_sample_zd"]),
                as_float64_contiguous(context_arrays["parent_sample_mstar"]),
                as_float64_contiguous(context_arrays["parent_sample_log_re"]),
                as_float64_contiguous(context_arrays["parent_sample_delta_r"]),
                as_float64_contiguous(context_arrays["base_normals"]),
                as_float64_contiguous(context_arrays["z_grid"]),
                as_float64_contiguous(context_arrays["chi_kpc_grid"]),
                as_float64_contiguous(context_arrays["population_gamma_axis"]),
                as_float64_contiguous(context_arrays["population_zd_axis"]),
                as_float64_contiguous(context_arrays["population_log_re_kpc_axis"]),
                as_float64_contiguous(context_arrays["population_n_axis"]),
                as_float64_contiguous(context_arrays["population_sigma_unit_grid"]),
                as_float64_contiguous(context_arrays["cs_theta_e_axis"]),
                as_float64_contiguous(context_arrays["cs_gamma_axis"]),
                as_float64_contiguous(context_arrays["cs_cross_section_grid"]),
                as_float64_contiguous(mass_bin_edges),
                as_float64_contiguous(sigma_star_bin_edges),
                as_float64_contiguous(log_re_bin_edges),
                as_float64_contiguous(delta_r_bin_edges),
                1 if is_sigma_star_gamma_model else 0,
            )
        )

    arrays = concatenate_chunk_outputs(chunk_outputs, _SONNENFELD_DIAGNOSTIC_ARRAY_NAMES)
