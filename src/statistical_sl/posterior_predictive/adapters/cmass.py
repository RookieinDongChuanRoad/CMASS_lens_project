...
from statistical_sl.models.cmass.posterior import (
...
def _run_shared_parent_diagnostics_numba(
    ...
    *,
    ...
    # The generic PPC layer resolves CPU width, while the package-owned Numba
    # backend applies the cap before RNG chunk allocation and kernel execution.
    # Keeping this in the backend namespace prevents adapters from depending on
    # inference-owned parallel helpers.
    ...
    for chunk_start, chunk_end in iter_draw_chunks(n_draws=n_draws, chunk_size=chunk_size):
        ...
        chunk_outputs.append(
            _shared_parent_diagnostics_numba_chunk(
                as_float64_contiguous(posterior_draws[chunk_start:chunk_end]),
                as_float64_contiguous(parent_normals),
                as_float64_contiguous(theta_uniforms),
                as_float64_contiguous(sigma_uniforms),
                as_float64_contiguous(sigma_noise),
                as_float64_contiguous(_diagnostics_scalar_context(context)),
                as_float64_contiguous(context.z_grid),
                as_float64_contiguous(context.chi_kpc_grid),
                ...
                as_float64_contiguous(sigma_arrays["n_axis"]),
                as_float64_contiguous(sigma_arrays["values"]),
    ...
    arrays = concatenate_chunk_outputs(chunk_outputs, _NUMBA_DIAGNOSTIC_ARRAY_NAMES)
