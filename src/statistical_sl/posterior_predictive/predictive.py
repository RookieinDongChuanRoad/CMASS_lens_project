...
from statistical_sl.inference.types import (
    ObservationRecord,
    ProfileSpec,
    RuntimeConfig,
)
...
DEFAULT_EXTERNAL_SIGMA_DIR = Path("workspace/data/external")
DEFAULT_PPC_OUTPUT_ROOT_DIR = Path("workspace/outputs")
...
def _load_ppc_runtime_config(config_path: str | Path) -> RuntimeConfig:
    """
    Load the current registry-driven runtime config for PPC.

    PPC diagnostics now share the same production parser as inference.  This is
    intentional: raw observation/cross-section snapshots are no longer a
    production fallback path, because accepting them here would bypass the
    canonical dataset boundary established by the new package layout.
    """
...
def _build_ppc_context(runtime_config: RuntimeConfig):
    """
    Build the numerical context used by PPC diagnostics.

    Current configs dispatch through the predictive registry so the generic PPC
    workflow no longer owns model-name branches.  The active model definition is
    responsible for building whatever context its diagnostics need, while this
    function preserves the tuple shape consumed by the artifact writers.
    """
...
def _reorder_sample_parameter_axis(
    ...
    *,
    ...
    """
    Return stored posterior samples in the run config's parameter order.

    Self-describing posterior artifacts may store parameter sites in a different
    order from the current runtime config.  Reordering at the loader boundary
    keeps all downstream predictive code unchanged and prevents silent
    parameter swaps.
    """
...
def _load_flattened_samples_npz(
    samples_path: Path,
    *,
    parameter_names: tuple[str, ...] | None,
) -> np.ndarray:
    """Load flattened posterior draws from a compact `samples.npz` artifact."""
    ...
    ordered = _reorder_sample_parameter_axis(
        samples_by_chain,
...
def _load_flattened_posterior_nc(
    posterior_path: Path,
    *,
    parameter_names: tuple[str, ...] | None,
) -> np.ndarray:
...
def _load_posterior_draws(
    ...
    """
    Load posterior draws from supported posterior artifacts.

    The current production inference path writes the emcee `chain.h5` artifact.
    If self-describing `samples.npz` or `posterior.nc` files are present in a
    run directory, they are treated as explicit posterior draw artifacts and are
    loaded before falling back to the chain.  Burn-in is only applied to
    `chain.h5`, because compact posterior artifacts are expected to contain
    already-selected posterior draws.
    """
    ...
    if samples_path.exists():
        flattened_draws = _load_flattened_samples_npz(
            samples_path,
            parameter_names=parameter_names,
        )
        artifact_name = "samples.npz"
    elif posterior_path.exists():
        flattened_draws = _load_flattened_posterior_nc(
            posterior_path,
            parameter_names=parameter_names,
        )
...
def annotate_existing_fig8_like_figures_with_observations(
    ...
    for profile_name, run_dir, diagnostics_dir in _resolve_requested_annotation_runs(resolved_outputs_root, run_dirs):
        ...
        try:
            ...
            # backend bridge instead of a raw observation compiler.
            compiled_context, profile_spec, _, cosmology, _, observations = _build_ppc_context(runtime_config)
