...
from statistical_sl.core.mass_definition import validate_h_ref, validate_unit_convention


DEFAULT_OUTPUT_ROOT = Path("workspace/outputs")
...
def _reject_removed_model_sections(raw_data: dict) -> None:
    ...
    around would make it ambiguous whether the registry or retired raw-config surface owns
...
def _load_parallel_strategy(runtime_raw: dict) -> str:
    """
    Normalize the runtime parallel strategy from YAML.

    YAML 1.1 parsers treat unquoted ``off`` and ``on`` as booleans.  Users are
    likely to write ``parallel_strategy: off`` naturally, so the loader maps
    that parsed boolean back to the intended string before the backend policy
    validates supported strategies.
    """

    raw_value = runtime_raw.get("parallel_strategy", "auto")
...
def _reject_removed_sampling_fields(sampling_raw: dict) -> None:
    """
    Reject removed sampling names before building `SamplingConfig`.
...
def load_runtime_config(config_path: str | Path) -> RuntimeConfig:
    ...
    return RuntimeConfig(
        ...
        runtime=RuntimeOptions(
            ...
            parallel_strategy=_load_parallel_strategy(runtime_raw),
