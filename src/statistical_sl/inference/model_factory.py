"""Adapt high-level model specs to the production Numba/emcee backend.
...
from statistical_sl.core.mass_definition import get_mass_definition
from statistical_sl.inference.parallel import resolve_parallelism
...
def _build_parameter_schema_for_spec(
    ...
    *,
    ...
    ``mass_definition`` is accepted for the registry callable shape.  The
    current ``ModelSpec`` already declares public names and bounds explicitly,
    so the argument is used only as a compatibility hook and does not rewrite
    the schema.
    """
...
def build_model_definition(
    model_spec: ModelSpec,
    runtime_adapter: ModelRuntimeAdapter,
    production_log_prob: Callable[[Any, CompiledModel, float], tuple[float, Any]],
) -> ModelDefinition:
    """
    ...
    entrypoints.  Model-owned posterior evaluation is passed as a callable, so
    the likelihood engine can stay generic while the registry remains the one
    place that binds a model name to its runtime and posterior adapter.
    """
