...
class DataConfig:
    """
    Input file locations used by inference and explicit validation utilities.

    `inference_dataset_path` is the only path loaded from production YAML.  The
    raw observation fields remain on the dataclass only for code that constructs
    a `RuntimeConfig` explicitly, such as historical artifact annotation.  The
    config parser does not accept those fields as a production input surface.
    """
