"""
emcee sampler orchestration for the production Numba/emcee backend.

The scientific hot path lives in
``statistical_sl.inference.backends.numba_emcee.likelihood_engine``.  This
module owns only sampler mechanics: walker initialization, optional
process-pool evaluation, checkpoint callbacks, run-log timing summaries, and
the persistent ``chain.h5`` HDFBackend contract.
"""
...
from statistical_sl.inference.outputs import append_run_log
from statistical_sl.inference.parallel import apply_thread_limits
from statistical_sl.inference.types import HyperParams, RuntimeContext
