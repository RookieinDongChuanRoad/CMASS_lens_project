"""Reusable interfaces for scientific strong-lensing model declarations.

This module is the package-owned home for the small contracts that model
authors use when declaring a statistical lensing model.  The contracts are
intentionally descriptive rather than executable: they name sampled
parameters, required data capabilities, backend keys, and context-packing
rules, while inference-specific execution still lives in the active backend.

The runtime-facing dataclasses live in ``statistical_sl.inference.types``.
Keeping the import direction explicit here avoids an extra adapter layer: model
authors can declare a model contract in this package while the inference
backend still owns execution containers such as ``RuntimeConfig`` and
``CompiledModel``.
"""
...
from statistical_sl.inference.types import CompiledModel, RuntimeConfig
from statistical_sl.models.parameter_schema import ParameterSchema
...
class ParameterSpec:
    ...
    output metadata.  Keeping this declaration small lets future model authors
    define a new inference target without learning how the backend serializes
    schemas.
    """
...
class ModelSpec:
    ...
    the backend implementation key.  It deliberately excludes compiled-context
    construction, sampler wiring, output writing, and registry boilerplate;
    those are framework concerns handled by ``ModelRuntimeAdapter`` and the
...
class ContextArraySpec:
    ...
    ``target_name`` is the attribute exposed on a backend-owned context when
    that backend needs a separate packed object.  Most fields keep the same
    name; the optional target lets future models use cleaner runtime names
    without changing their HDF5-normalized source objects.
    """
...
class CompiledContextBundle:
    ...
    The generic backend uses ``context`` to build ``CompiledModel.context``,
    while the remaining objects preserve the existing metadata and output
    contracts.
    """
