...
```bash
conda run -n cmass_lens python -m interpolation_grids --build-sigma-unit-hdf5 --profile all --observation-flavor all --sigma-definition all --workers 14
```
...
```bash
conda run -n cmass_lens python -m interpolation_grids --build-sigma-unit-hdf5 --profile all --sigma-definition within_re --workers 14
```
