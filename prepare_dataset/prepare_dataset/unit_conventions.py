"""Unit-convention helpers for interpolation-grid preparation.

The preparation package is the first place where raw physical observables are
translated into serialized HDF5 grids. Keeping the h-dependent algebra here
serves two purposes:
- grid builders can write native `h_units_v1` products instead of relying on
  downstream inference-time reinterpretation
- migration tests can compare old fixed-kpc products against the exact analytic
  conversion without duplicating formulas in several modules
"""
